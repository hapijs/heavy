...
internals.Policy.prototype.check = function () {
    ...
    return (!this.settings.maxEventLoopDelay || load.eventLoopDelay <= this.settings.maxEventLoopDelay) &&
           (!this.settings.maxHeapUsedBytes || load.heapUsed <= this.settings.maxHeapUsedBytes) &&
           (!this.settings.maxRssBytes || load.rss <= this.settings.maxRssBytes);
};